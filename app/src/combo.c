...
static int capture_pressed_key(const zmk_event_t *ev) {
    ...
    memcpy(&pressed_keys[pressed_keys_count++], ev,
           sizeof(struct zmk_position_state_changed_event));
...
static int position_state_up(const zmk_event_t *ev, struct zmk_position_state_changed *data) {
    ...
    if (released_keys > 1) {
        ...
        memcpy(&dupe_ev, ev, sizeof(struct zmk_position_state_changed_event));
